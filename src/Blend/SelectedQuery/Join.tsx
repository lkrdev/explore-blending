...
const Join = ({
  ...
  const { selectedQuery, queries, joins, validateJoin, updateJoinType, selectQuery } = useBlendContext();
  ...
  if (hidden) {
    ...
    return <Popover
      ...
      id="join-popover"
      content={<PopoverLayout header="Update Joins" footer >
        <SpaceVertical width="100%">
          <JoinTypeSelect
            current={currentJoinType}
            updateJoinType={(join_type: TJoinType) => updateJoinType(currentJoinData.to_query_id, join_type)}
          />
          {current_query_joins.map((j, index) => (
            <JoinRow
              ...
              join={j}
              index={index}
              is_last={index === current_query_joins.length - 1}
    ...
    >
      <IconButton
        ...
        disabled={disabled}
        toggle={false}
        icon={<JoinInner color={valid_joins.every(Boolean) ? undefined : theme.colors.critical} size={18} />}
        label="Update Joins"
      />
    </Popover>
  }
  ...
  //           ))
  //         ) : (
  //           // Render the default row when current_query_joins is empty
  //           <JoinRow
  ...
  //     <Box height={Math.max(0, (current_query_joins.length > 0 ? current_query_joins.length : 1) * 36 - 24)} />
  //   </>
  // );
};
