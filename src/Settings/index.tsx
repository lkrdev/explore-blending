...
const Settings: React.FC<ConfigModalProps> = ({ isOpen, onClose }) => {
    const { user, is_admin, checkEntitlements } = useAppContext();
    ...
    const handleValidate = async (values: typeof initialValues) => {
        ...
        if (values.override_api?.length) {
            if (!values.override_api.startsWith('http')) {
                errors.override_api =
                    'Override API must start with http or https';
            }
            if (!values.override_api.endsWith('/api')) {
                errors.override_api = 'Override API must end with /api';
            }
            if (
                !window.lookerExtensionMetadata.entitlements.external_api_urls
                    .map((url: string) => {
                        const u = new URL(url).origin;
                        return u;
                    })
                    .includes(new URL(values.override_api).origin)
            ) {
                errors.override_api =
                    'Override API needs to be added to the extension entitlement for external_api_urls';
            }
        }
        return errors;
