...
import { Entitlements, required_entitlements } from './entitlements';
...
interface IAppContext {
    ...
    checkEntitlements: () => {
        ...
        required_entitlements: Partial<Entitlements>;
    };
}
...
export const AppContextProvider = ({
    ...
    const getExploreFields = async (explore_id: string) => {
        if (explore_id) {
            ...
            try {
                ...
                console.warn(
                    'Failed to fetch explore fields for:',
                    explore_id,
                    error
                );
                return {};
    ...
    const loadCachedModels = async () => {
        const cached_models = await extension.localStorageGetItem(
            'models:0.0.2'
        );
        let model_count = 0;
        let explore_count = 0;
        ...
        if (models?.length) {
            ...
            if (cached_model_connections.refresh) {
                ...
                const all_model_connections = await Promise.all(
                    models.map(async (model) => {
                        ...
                        while (model.explores.length > 0 && !retrieved_model) {
                            ...
                            try {
                                const connection =
                                    await getModelConnectionPromise(
                ...
                setStatus((p) => [
                    ...
                    } model connections from the server`,
                ]);
            }
            setStatus((p) => {
                if (
    ...
    const checkEntitlements = () => {
        let errors: string[] = [];
        const current_entitlements =
            window.lookerExtensionMetadata.entitlements;
        (
            Object.keys(required_entitlements) as Array<keyof Entitlements>
        ).forEach((method) => {
            const required = required_entitlements[method];
            const current = current_entitlements[method];

            if (Array.isArray(required)) {
                // For array entitlements, check if all required items are included
                if (
                    ...
                    if (missing.length > 0) {
                        errors.push(
                            `Missing entitlements from list: ${method} (${missing.join(
                                ', '
                            )})`
                        );
                    }
                ...
                // For boolean entitlements, check if it's true
                if (current !== true) {
                    errors.push(`Missing entitlement: ${method} (${required})`);
                }
            } else if (required !== null && required !== undefined) {
                // For other types, check if it exists
                if (!current) {
                    errors.push(`Missing entitlement: ${method} (${required})`);
                }
            }
        });
        return { errors, current_entitlements, required_entitlements };
    };
