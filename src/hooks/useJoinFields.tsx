...
const useJoinFields = (evaluate_query: IQuery) => {
    ...
    const { to_fields, from_fields } = useMemo(() => {
            ...
            forEach(query.fields, (field) => {
                ...
                if (found_field) {
                    if (in_current_query) {
                        ...
                        const groupLabel = getExploreLabelFromFieldWithIndex(
                            query,
                            index
                        );
                        const current_group = from_fields_calc.find(
        ...
        // Sorting logic
        from_fields_calc = from_fields_calc.map((group) => ({
            ...group,
        ...
        return {
            to_fields: to_fields_calc.sort((a, b) =>
                String(a.label).localeCompare(String(b.label))
            ),
            from_fields: from_fields_calc.sort((a, b) =>
                String(a.label).localeCompare(String(b.label))
            ),
        };
